...
# сильно перемешанные 5s5p⁶ и 5p⁴5d — одним КВ-расчётом; корреляционная
# орбиталь 6d (dbsr_mchf по физическим состояниям) даёт термозависимость 5d.
# В результате остаются только физические состояния, все ортогональны.
tg.add(["5s 5p6", "5s2 5p4 5d"], correlation=["5s2 5p4 6d"])
tg.add("5s2 5p4 6s")
tg.add("5s2 5p4 6p")
